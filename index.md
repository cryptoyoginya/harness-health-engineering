# Index — карта KB

> Навигационная карта. При добавлении артефакта — добавь ссылку сюда (шаг ingest из AGENTS.md).

## Контекст

- [Домен «Здоровье»](./00_context/health.md) — что это, роль Whoop, измерения, медицинская граница
- [Метрики (evals)](./00_context/health-metrics.md) — ведущие/запаздывающие + north-star
- [Semantic invariant](./.remember/core.md) — цель, контекст, hard rules

## Источники

- [Недельные source-notes](./02_sources/health/) — разметка FACT/INFERENCE по неделям

## Wiki (концепты)

- [Здоровье — бейзлайны и история](./03_wiki/health.md) — личный HRV, травмы, таксономия активностей
- [Реестр стека бадов](./03_wiki/health-supplements.md) — что/доза/тайминг/цель/статус

## Synthesis

- [Life quality — north-star](./04_synthesis/life-quality.md) ⭐ главная метрика
- [Недельный синтез (шаблон)](./04_synthesis/health-weekly-template.md)
- [Open questions](./04_synthesis/open-questions.md) — известные пробелы
- [Contradictions](./04_synthesis/contradictions.md)

## Decisions

- [Эксперименты](./05_decisions/experiments/) — одна переменная за раз
- [EXP-001: отсечка кофеина в 14:00](./05_decisions/experiments/EXP-001-caffeine-cutoff.md)

## Skills (плейбуки)

- [Красный recovery](./skills/health-red-recovery.md)
- [Долг сна](./skills/health-sleep-debt.md)
- [Спад настроения](./skills/health-mood-dip.md)
- [«Это я или биология?» — атрибуция спада](./skills/health-attribution.md) ⭐ killer-фича
- [Life review — «стало ли лучше жить»](./skills/health-life-review.md) ⭐ north-star
- [Травма / боль](./skills/health-injury.md)
- [Возврат в режим](./skills/health-getting-back.md)
- [Недельный синтез](./skills/health-weekly-synthesis.md)

## Whoop ingest

- [scripts/whoop/](./scripts/whoop/README.md) — авто-ingest физиологии в daily-логи

---

## Журнал

См. [`log.md`](./log.md) — хронология ingest-операций и больших правок.
